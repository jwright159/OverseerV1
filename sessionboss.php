...
function convertHybridb($workrow, $isbodygear)
...
if (empty($_SESSION['username'])) {
	...
	while ($buddyrow = $sessionmates->fetch_array()) {
		...
		if ($buddyrow['combatconsume'] == 1)
			echo "Consumable action used!</br>";
		echo '</div>';
	}
	echo "</div>";
	...
	if ($userrow['dreamingstatus'] == "Prospit") {
		...
		if ($userrow['lastactive'] == "aggrieve")
			echo '<option value="aggrieve" selected>"AGGRIEVE" (' . $aggrieve . ')</option>';
		else
			echo '<option value="aggrieve">"AGGRIEVE" (' . $aggrieve . ')</option>';
		if ($userrow['lastactive'] == "aggress")
			echo '<option value="aggress" selected>"AGGRESS" (' . $aggress . ')</option>';
		else
			echo '<option value="aggress">"AGGRESS" (' . $aggress . ')</option>';
		if ($userrow['lastactive'] == "assail")
			echo '<option value="assail" selected>"ASSAIL" (' . $assail . ')</option>';
		else
			echo '<option value="assail">"ASSAIL" (' . $assail . ')</option>';
		if ($userrow['lastactive'] == "assault")
		...
		if ($userrow['lastpassive'] == "abuse")
			...
			echo '<option value="abuse">"ABUSE" (' . $abuse . ')</option>';
		if ($userrow['lastpassive'] == "accuse")
			echo '<option value="accuse" selected>"ACCUSE" (' . $accuse . ')</option>';
		else
			echo '<option value="accuse">"ACCUSE" (' . $accuse . ')</option>';
		if ($userrow['lastpassive'] == "abjure")
			echo '<option value="abjure" selected>"ABJURE" (' . $abjure . ')</option>';
		else
			echo '<option value="abjure">"ABJURE" (' . $abjure . ')</option>';
		if ($userrow['lastpassive'] == "abstain")
  ...
  if ($userrow['dreamingstatus'] == "Prospit") {
	...
	if (!empty($_SESSION['adjective']))
		echo " | <a href='aspectpowers.php'>DO THE $_SESSION[adjective] THING</a> | <a href='roletech.php'>Peruse and select roletechs</a>";
