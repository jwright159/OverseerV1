  ...
  if (!empty($_POST['usepattern'])) { //User accessing a pattern.
	...
	if ($userrow['enemy1name'] != "" || $userrow['enemy2name'] != "" || $userrow['enemy3name'] != "" || $userrow['enemy4name'] != "" || $userrow['enemy5name'] != "" || $userrow['aiding'] != "") { //Player strifing.
	  ...
	  if ($targetrow['session_name'] != $userrow['session_name']) {
	    ...
	    if ($patternrow[$aspectvialstr] != 0) $mysqli->query("UPDATE `Ability_Patterns` SET `aspectvialuses` = $patternrow[aspectvialuses]+1 WHERE `Ability_Patterns`.`username` = '$username' LIMIT 1;"); //Check for whether the aspect vial reduction option was used.
		...
		$abilities = $mysqli->query("SELECT * FROM `Abilities` WHERE `Abilities`.`Aspect` IN ('$userrow[Aspect]','All') AND `Abilities`.`Class` IN ('$userrow[Class]','All') 
AND `Abilities`.`Rungreq` BETWEEN $userrow[Echeladder]+1 AND $userrow[Echeladder]+$rungs AND `Abilities`.`Godtierreq` = 0 ORDER BY `Abilities`.`Rungreq` DESC;");
		...
		$i++;
	      ...
	      if ($patternrow[$maxtargetstr] > 1) $mysqli->query("UPDATE `Ability_Patterns` SET `multitargetuses` = $patternrow[multitargetuses]+1 WHERE `Ability_Patterns`.`username` = '$username' LIMIT 1;"); //Multiple targets selected, increment multitarget usage.
		...
		if ($striferow[$enemystr] != "") {
		  ...
		  if ($patternrow[$damagestr] > 0) {
		    ...
		    if ($j == 1) $mysqli->query("UPDATE `Ability_Patterns` SET `damageuses` = $patternrow[damageuses]+1 WHERE `Ability_Patterns`.`username` = '$username' LIMIT 1;");
		  ...
		  if ($patternrow[$powerdownstr] > 0) {
		    if ($j == 1) $mysqli->query("UPDATE `Ability_Patterns` SET `powerdownuses` = $patternrow[powerdownuses]+1 WHERE `Ability_Patterns`.`username` = '$username' LIMIT 1;");
	    ...
	    if ($patternrow[$offenseupstr] != 0) $mysqli->query("UPDATE `Ability_Patterns` SET `offenseupuses` = $patternrow[offenseupuses]+1 WHERE `Ability_Patterns`.`username` = '$username' LIMIT 1;");
	    if ($patternrow[$defenseupstr] != 0) $mysqli->query("UPDATE `Ability_Patterns` SET `defenseupuses` = $patternrow[defenseupuses]+1 WHERE `Ability_Patterns`.`username` = '$username' LIMIT 1;");
	    if ($powerboost != 0) $mysqli->query("UPDATE `Ability_Patterns` SET `powerupuses` = $patternrow[powerupuses]+1 WHERE `Ability_Patterns`.`username` = '$username' LIMIT 1;");
	    if ($temp == False) {
	      if ($patternrow[$offenseupstr] != 0) $mysqli->query("UPDATE `Players` SET `offenseboost` = $targetrow[offenseboost]+$patternrow[$offenseupstr] WHERE `Players`.`username` = '$target'");
	      if ($patternrow[$defenseupstr] != 0) $mysqli->query("UPDATE `Players` SET `defenseboost` = $targetrow[defenseboost]+$patternrow[$defenseupstr] WHERE `Players`.`username` = '$target'");
	      if ($powerboost != 0) $mysqli->query("UPDATE `Players` SET `powerboost` = $targetrow[powerboost]+$powerboost WHERE `Players`.`username` = '$target'");
	    } else { //Temp boosted.
	  ...
	  if ($patternrow[$damagestr] != 0 || $patternrow[$powerdownstr] != 0 || $patternrow[$temporarystr] != 0 || $patternrow[$invulnstr] != 0) { //Ability only usable during combat.
	    ...
	    if ($targetrow['session_name'] != $userrow['session_name']) {
	      ...
	      if ($patternrow[$aspectvialstr] != 0) $mysqli->query("UPDATE `Ability_Patterns` SET `aspectvialuses` = $patternrow[aspectvialuses]+1 WHERE `Ability_Patterns`.`username` = '$username' LIMIT 1;"); //Check aspect vial reduction.
	      $powerboost = 0;
	      if ($patternrow[$offenseupstr] >= $patternrow[$defenseupstr]) {
		$powerboost = $patternrow[$defenseupstr];
		$patternrow[$offenseupstr] = $patternrow[$offenseupstr] - $patternrow[$defenseupstr];
