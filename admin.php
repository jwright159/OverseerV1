	...
	if ($userrow['admin'] == 0) {
		...
		if (!empty($_POST['exile'])) { //Attempting to exile
			...
			if ($exilerow['session_name'] != $userrow['session_name'] || $exilerow['username'] == $sessionrow['admin'] || $exilerow['username'] == $userrow['username']) { //Target is either the primary admin or not in the session
				...
				while ($sessionrow = $sessionmates->fetch_array()) {
					if ($sessionrow['server_player'] == $exilerow['username'])
						$mysqli->query("UPDATE `Players` SET `server_player` = '' WHERE `Players`.`username` = '$sessionrow[username]'");
					if ($sessionrow['client_player'] == $exilerow['username'])
						$mysqli->query("UPDATE `Players` SET `client_player` = '' WHERE `Players`.`username` = '$sessionrow[username]'");
				}
		...
		if (!empty($_POST['unexile'])) { //Attempting to un-exile
			...
			if ($unexilerow['former_session_name'] != $userrow['session_name']) { //Target not originally from this session
				...
				while ($sessionrow = $sessionmates->fetch_array()) {
					if ($sessionrow['server_player'] == $unexilerow['username'])
						$mysqli->query("UPDATE `Players` SET `server_player` = '' WHERE `Players`.`username` = '$sessionrow[username]'");
					if ($sessionrow['client_player'] == $unexilerow['username'])
						$mysqli->query("UPDATE `Players` SET `client_player` = '' WHERE `Players`.`username` = '$sessionrow[username]'");
				}
		...
		if (!empty($_POST['forceclient']) && !empty($_POST['forceserver'])) { //Attempting to force a server-client connection.
			...
			if ($allow) { //Targets valid
				...
				while ($sessionrow = $sessionmates->fetch_array()) {
					if ($sessionrow['server_player'] == $serverrow['username'])
						$mysqli->query("UPDATE `Players` SET `server_player` = '' WHERE `Players`.`username` = '$sessionrow[username]'");
					if ($sessionrow['client_player'] == $clientrow['username'])
						$mysqli->query("UPDATE `Players` SET `client_player` = '' WHERE `Players`.`username` = '$sessionrow[username]'");
				}
		...
		if ($sessionrow['admin'] == $username) {
			...
			if (!empty($_POST['settingschange'])) {
				if (!empty($_POST['randoms'])) {
					if ($_POST['randoms'] == "yes")
						$gorandom = "1";
					else
						$gorandom = "0";
				} else
					$gorandom = strval($sessionrow['allowrandoms']);
				if (!empty($_POST['unique'])) {
					...
					$gounique = strval($sessionrow['uniqueclasspects']);
