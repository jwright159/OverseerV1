...
AND `Abilities`.`Rungreq` BETWEEN 0 AND $userrow[Echeladder] AND `Abilities`.`Godtierreq` BETWEEN 0 AND $userrow[Godtier] ORDER BY `Abilities`.`Rungreq` DESC;");
	...
	if (!empty($_POST['usemotif'])) {
		...
		if ($userrow['fraymotifuses'] == 0) { //User is out of fraymotifs
			...
			if ($motif == "Blood(motif)")
				$motif = "Blood"; //Turn blood back so we can check for it properly.
								...
								if (!empty($userrow[$namestr])) {
									...
									if ($damage > 0) {
										...
										if ($monsterhealth[$i] < 1)
											$monsterhealth[$i] = 1; //Needs to be done this way because berserk ALSO inflicts damage.
										$userrow[$healthstr] = $newhealth;
									}
								}
								$i++;
	...
	if (empty($userrow['Aspect'])) {
		...
		if ($userrow['enemydata'] != "") {
			...
			while ($col = $motifresult->fetch_field()) {
				...
				if ($motifaspect == "Blood")
					$motifaspect = "Blood(motif)"; //Hack -- Blood is already used for blood grist, so we need a new name.
				if (empty($newmotif))
					$newmotif = "";
