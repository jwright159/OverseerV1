		...
		if (empty($landrow)) { //A land row was not found. This is probably because the player selected a special location.
					...
					while ($enemies <= $max_enemies) {
						...
						while ($row = $result2->fetch_array()) {
							...
							if ($row['appearson'] == "Prospit") {
								if ($userrow['olddreamenemy' . strval($enemies)] == $enemytype)
									echo '<option value="' . $enemytype . '" selected>' . $enemytype . ' ("Power": ' . $row['basepower'] . ')</option>'; //Produce an option in the dropdown menu for this type of enemy.
								else
					...
					while ($enemies <= $max_enemies) {
						...
						while ($row = $result2->fetch_array()) {
							...
							if ($row['appearson'] == "Battlefield") {
								if ($userrow['oldenemy' . strval($enemies)] == $enemytype)
									echo '<option value="' . $enemytype . '" selected>' . $enemytype . ' (Power: ' . $row['basepower'] . ')</option>'; //Produce an option in the dropdown menu for this type of enemy.
								else
									echo '<option value="' . $enemytype . '">' . $enemytype . ' (Power: ' . $row['basepower'] . ')</option>'; //Produce an option in the dropdown menu for this type of enemy.
							}
			...
			while ($enemies <= $max_enemies) {
				...
				while ($i < 10) {
					...
					if ($userrow['oldgrist' . strval($enemies)] == $gristrow[$griststr])
						echo '<option value="' . $gristrow[$griststr] . '" selected>' . $gristrow[$griststr] . ' (x' . strval($i) . ')</option>'; //Produce an entry for this grist type.
					else
				...
				while ($row = $result2->fetch_array()) {
					...
					if ($row['appearson'] == "Lands") {
						if ($userrow['oldenemy' . strval($enemies)] == $enemytype)
							echo '<option value="' . $enemytype . '" selected>' . $enemytype . ' (Base power: ' . $row['basepower'] . ')</option>'; //Produce an option in the dropdown menu for this type of enemy.
						else
