...
if (empty($_SESSION['username'])) {
	...
	if ($explorow['boonreward'] != 0)
		$mysqli->query("UPDATE `Players` SET `Boondollars` = " . strval($userrow['Boondollars'] + $explorow['boonreward']) . " WHERE `Players`.`username` = '$username' LIMIT 1 ;");
	$i = 1;
	while ($i <= $max_links) {
		...
		if (!empty($explorow[$linkstr])) {
			if (linkchecker($userrow, $explorow, $i)) { //Check to see if user gets this link.
				...
				if (!empty($_POST['newdesc']))
					echo '<input type="hidden" name="olddesc" value="' . $_POST['newdesc'] . '" />';
