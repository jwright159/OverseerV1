		...
		while ($enemies > 0) {
			...
			while (($potentialrow = $potentialresult->fetch_array()) && $options > 0) {
				...
				if ($selected) {
					$options = 0;
					$tier = 1;
					while (($potentialrow['basepower'] * $tier) <= $realmax && $tier < 10)
				...
				while ($i <= 10) {
					while ($j <= 10) {
						...
						if (!empty($roomarray[$tile]))
							$mysqli->query("UPDATE `Dungeons` SET `$tile` = '" . $mysqli->real_escape_string($roomarray[$tile]) . "' WHERE `Dungeons`.`username` = '$username' LIMIT 1;");
						$j++;
					}
					$j = 1;
					$i++;
