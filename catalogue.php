  ...
  if ($userrow['dreamingstatus'] != "Awake") {
    ...
    if (!empty($_POST['catalogue'])) { //User captchalogues item.
      ...
      if ($userrow['captchalogues'] < $captchas) {
      	if ($challenge == 1) {
      		$totalitems = 0;
      		if ($userrow['captchalogues'] == 0) $itemsresult = $mysqli->query("SELECT `name` FROM `Captchalogue` WHERE `Captchalogue`.`catalogue` = 1 AND `Captchalogue`.`abstratus` NOT LIKE '%notaweapon%' ;"); //user is guaranteed to get a weapon on their first try
      		elseif ($userrow['captchalogues'] == 1) $itemsresult = $mysqli->query("SELECT `name` FROM `Captchalogue` WHERE `Captchalogue`.`catalogue` = 1 AND `Captchalogue`.`abstratus` LIKE '%computer%' ;"); //user is guaranteed to get a computer on their second try
      		else $itemsresult = $mysqli->query("SELECT `name` FROM `Captchalogue` WHERE `Captchalogue`.`catalogue` = 1");
      		while ($row = $itemsresult->fetch_array()) $totalitems++;
      		$item1 = rand(1,$totalitems);
      		$item1--;
      		if ($userrow['captchalogues'] == 0) $result = $mysqli->query("SELECT * FROM Captchalogue WHERE `Captchalogue`.`catalogue` = 1 AND `Captchalogue`.`abstratus` NOT LIKE '%notaweapon%' LIMIT $item1 , 1 ;");
      		elseif ($userrow['captchalogues'] == 1) $result = $mysqli->query("SELECT * FROM Captchalogue WHERE `Captchalogue`.`catalogue` = 1 AND `Captchalogue`.`abstratus` LIKE '%computer%' LIMIT $item1 , 1 ;");
      		else $result = $mysqli->query("SELECT * FROM Captchalogue WHERE `Captchalogue`.`catalogue` = 1 LIMIT $item1 , 1 ;");
      	} else {
	$result = $mysqli->query("SELECT * FROM Captchalogue WHERE `Captchalogue`.`captchalogue_code` = '" . $_POST['catalogue'] . "'");
      	}
    ...
    $result = $mysqli->query ("SELECT * FROM Captchalogue WHERE catalogue = 1 AND refrance = 0 ORDER BY name");
    while ($row = $result->fetch_array()) {
