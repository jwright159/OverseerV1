...
if (empty($_SESSION['username'])) {
	...
	if (strpos($userrow['storeditems'], "USELESS.") !== false)
		...
		$useless = false;
	if (strpos($userrow['storeditems'], "RECYCLER.") !== false)
		$recycler = true;
	else
		$recycler = false;
		...
		if (!$canon || strpos($userrow['storeditems'], "CAPTCHASCAN.") !== false) {
			...
			while ($itemrow = $itemresult->fetch_array()) {
				...
				if ($itemname == $userrow[$invslot] || $ghostmode) {
					if (strpos($userrow['storeditems'], "LASERSTATION.") === false && strpos($itemrow['effects'], "OBSCURED|") !== false)
						echo "Not even your fancy-pants remote captchalogue scanner can read the code on the back of that first item!</br>";
					else
						$_POST['code1'] = $itemrow['captchalogue_code'];
				}
			}
			...
			while ($itemrow = $itemresult->fetch_array()) {
				...
				if ($itemname == $userrow[$invslot] || $ghostmode) {
					if (strpos($userrow['storeditems'], "LASERSTATION.") === false && strpos($itemrow['effects'], "OBSCURED|") !== false)
						echo "Not even your fancy-pants remote captchalogue scanner can read the code on the back of that second item!</br>";
					else
						$_POST['code2'] = $itemrow['captchalogue_code'];
				}
			}
		...
		if (!$canon || $camefromscanner || strpos($userrow['storeditems'], "CAPTCHACOMBINE.") !== false) {
			...
			if ($challenge == 1) {
				...
				while ($itemrow1 = $itemresult1->fetch_array()) {
					if (!(strrpos($sessionrow['atheneum'], $itemrow1['captchalogue_code']) === false))
						$skip1 = true;
				}
				...
				while ($itemrow2 = $itemresult2->fetch_array()) {
					if (!(strrpos($sessionrow['atheneum'], $itemrow2['captchalogue_code']) === false))
						$skip2 = true;
				}
				if ($skip1 && $skip2)
					$letthrough = true;
			...
			if ($userrow['Build_Grist'] >= 4) {
				...
				if ($letthrough == true) {
					...
					echo "You can't combine codes that aren't in your atheneum in Challenge Mode. If the code(s) you tried to combine is/are in your inventory, then it is bugged somehow. Use the populate atheneum page to fix it.</br>";
			} else {
	...
	if (!empty($_POST['holoitem'])) {
		if (strpos($userrow['storeditems'], "HOLOPAD.") !== false) {
			if (strpos($userrow[$_POST['holoitem']], "Cruxite Dowel (CODE:") !== false || strpos($userrow[$_POST['holoitem']], "Captchalogue Card (CODE:") !== false) {
				...
				echo "That object is not compatible with the Holopad!</br>";
		} else
	...
	if (!empty($_GET['holocode'])) { //User is using the holopad.
		...
		if ($combined) {
			if ($_POST['combine'] == "or")
				$recipetext = "&newrecipe=" . $itemrow1['name'] . "%20||%20" . $itemrow2['name'];
			else
	...
	if (!empty($_POST['alchcrux'])) {
		if (strpos($userrow['storeditems'], "ALCHEMITER.") !== false) {
			if (strpos($userrow[$_POST['alchcrux']], "Cruxite Dowel (CODE:") !== false) {
				...
				echo "That object is not compatible with the alchemiter!</br>";
		} else
			echo "You don't have a device that can alchemize items!</br>";
	}

	if (!empty($_POST['alchcode'])) { //User is using the alchemiter.
		$letthrough = false;
		if ($challenge == 1) {
			if (strrpos($sessionrow['atheneum'], $_POST['alchcode']) === false)
				$letthrough = false;
			else
				$letthrough = true;
		} else
		...
		if ($letthrough) {
			...
			if (empty($_POST['autostore']) && $userrow['sessionbossengaged'] == 0) {
				...
				if ($userrow['sessionbossengaged'] != 0)
					echo "Unfortunately you're kind of too busy to pick up any items right now, so they just sit around on the Alchemiter pad until some helpful consorts cart them into your house.</br>";
			...
			if ($itemfound == True && $canafford == True) { //Player successfully creates item.
				...
				while ($n < $numberalched) { //earlier code should prevent making the item if not enough space for it
					...
					if ($itemslot != "inv-1")
						$userrow[$itemslot] = $itemrow['name']; //update this so using the userrow again will not overwrite the item we just added
					else
						$tostorage++; //if no room, add one to the amount we need to send to storage
					$n++;
				}
				...
				if (strpos($itemrow['abstratus'], "bladekind") !== false && $userrow['down'] == 0 && $userrow['enemydata'] == "" && $userrow['aiding'] == "") { //random chance of summoning the blade cloud, gasp!
					...
					if ($bcchance < 1)
						$bcchance = 1; //min chance of 1% for weak weapons
	...
	if ($invspace > 0) {
		...
		while ($row = $captchalogueresult->fetch_array()) {
			...
			if ($highestactive > 0)
				echo "Highest active bonus: $highestactive</br>";
			...
			if ($highestpassive > 0)
				echo "Highest passive bonus: $highestpassive</br>";
