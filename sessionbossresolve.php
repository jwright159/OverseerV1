...
function refreshSingular($slot, $buddyrow)
...
if (empty($_SESSION['username'])) {
	...
	while ($buddyrow = $sessionmates->fetch_array()) {
		...
		if (!empty($someshittyvariablethatdoesntexist)) {
			...
			if (empty($buddyrow['Class']))
				$buddyrow['Class'] = "Default";
	...
	AND `Abilities`.`Rungreq` BETWEEN 0 AND $buddyrow[Echeladder] AND `Abilities`.`Godtierreq` BETWEEN 0 AND $buddyrow[Godtier] ORDER BY `Abilities`.`Rungreq` DESC;");
			...
			if ($buddyrow['headgear'] != "" && $buddyrow['dreamingstatus'] == "Awake") {
				...
				if ($headrow['hybrid'] == 1)
					$headrow = convertHybrid($headrow, false);
			...
			if ($buddyrow['facegear'] != "" && $buddyrow['facegear'] != "2HAND" && $buddyrow['dreamingstatus'] == "Awake") {
				...
				if ($facerow['hybrid'] == 1)
					$facerow = convertHybrid($facerow, false);
			...
			if ($buddyrow['bodygear'] != "" && $buddyrow['dreamingstatus'] == "Awake") {
				...
				if ($bodyrow['hybrid'] == 1)
					$bodyrow = convertHybrid($bodyrow, false);
			...
			if ($buddyrow['accessory'] != "" && $buddyrow['dreamingstatus'] == "Awake") {
				...
				if ($accrow['hybrid'] == 1)
					$accrow = convertHybrid($accrow, false);
				$accdef = $accrow['power'];
			} else {
				$accdef = 0;
			}
			...
			if (!empty($abilities[17])) { //Blood Bonds activates. Increase power according to the number of players fighting.
				...
				while ($testrow = $testsessionmates->fetch_array()) {
					if ($testrow['sessionbossengage'] == 1)
			...
			if (!empty($abilities[7])) { //Aspect Fighter (ID 7)
				...
				if ($offensebonus < 0)
					$offensebonus = 0; //Paranoia: If something goes wrong, set the value to zero.
				if ($defensebonus < 0)
					$defensebonus = 0;
			...
			if ($buddyrow[$enemystr] != "") { //Enemy spotted! (failsafe: If no fragment, do nothing.
				...
				if (!empty($abilities[23])) { //Fortune's Protection activates (ID 23). Regardless of other modifiers, grant a flat chance to score an instant critical. (Light guides your blow)
					...
					if ($guidance > (100 - floor($buddyrow['Echeladder'] / 50)))
				...
				if ($enemydamage < 0)
					$enemydamage = 0; //No healing enemies with attacks!
				$timestopped = False;
						...
						while (!empty($effectarray[$effectnumber])) {
							...
							switch ($currentarray[0]) {
								...
								case AFFINITY: //Format is AFFINITY:<affinity type>:<percentage>|
									if ($currentarray[1] == "All" && $buddyrow['aspect'] != "")
										$currentarray[1] == $buddyrow['aspect'];
									...
									if ($affinityfactor < -1)
										$affinityfactor = -1; //Maximum of 100% absorption.
				...
				if (!empty($noonecares)) { //NOTE - this always fails. Just easier than reformatting.
					...
					if (strpos($currentstatus, $timestopstr) !== False) { //This enemy is frozen in time.
						...
						if (strpos($individualstatus, $nocapstr) !== False) { //Player's massive damage cap is gone.
							...
							if ($playerdamage > ($buddyrow['Gel_Viscosity'] / 2.5))
								$playerdamage = floor($buddyrow['Gel_Viscosity'] / 2.5); //Massive damage safety net.
						}
						if (empty($meteordodged))
							$playerdamage += $meteordamage; //Being hit by a meteor pisses you off!
				...
				if ($buddyrow[$healthvialstr] - $damage < $buddyrow['Gel_Viscosity']) {
					...
					if ($newhealth - $recoildamage < 0)
						$newhealth = 1; //recoil shouldn't kill the player
					else
						$newhealth -= $recoildamage;
				...
				if ($buddyrow['motifcounter'] == 0 || $buddyrow['Aspect'] != "Time") { //Time III prevents boosts from ticking down.
					if ($buddyrow['temppowerduration'] > 0) {
						...
						if ($repeat)
							$powerlevel = $powerlevel - $buddyrow['temppowerboost']; //Remove boost in the case that we're repeating.
					...
					if ($buddyrow['tempoffenseduration'] > 0) {
						...
						if ($repeat)
							$offensepower = $offensepower - $buddyrow['tempoffenseboost']; //Remove boost in the case that we're repeating.
					...
					if ($buddyrow['tempdefenseduration'] > 0) {
						...
						if ($repeat)
							$defensepower = $defensepower - $buddyrow['tempdefenseboost']; //Remove boost in the case that we're repeating.
				...
				if ($buddyrow['motifcounter'] > 0) { //Player's tier 3 fraymotif is active. The effect of the fraymotif is stored and performed here. "motifvar" is a wildcard variable that may or
					...
					switch ($buddyrow['Aspect']) {
						case "Breath": //Breathless Battaglia
							if (($buddyrow['motifcounter'] % 5) != 0) { //Drain power.
								...
								if (!empty($buddyrow[$enemystr])) {
									...
									if ($powerdrain > $buddyrow[$powerstr])
										$powerdrain = $buddyrow[$powerstr];
								...
								if ($damage > $buddyrow[$healthstr])
									$damage = $buddyrow[$healthstr] - 1;
						...
						case "Heart":
							...
							if ($newaspect > $buddyrow['Gel_Viscosity'])
								$newaspect = $buddyrow['Gel_Viscosity'];
						...
						case "Life":
							...
							if ($newhealth > $buddyrow['Gel_Viscosity'])
								$newhealth = $buddyrow['Gel_Viscosity'];
							//$mysqli->query("UPDATE `Players` SET `$healthvialstr` = $newhealth WHERE `Players`.`username` = '$username' LIMIT 1 ;");
						...
						case "Space":
							$zillystr = "";
							if (empty($luck))
								$luck = 0; //Paranoia: If something dumb happens to luck, make it zero. Negatives are fine since rand can handle them anyhow.
							$zilly = rand((1 + floor($luck / 3)), 100); //Max luck increases chance of zillyweapon.
							if ($zilly < 1)
								$zilly = 1; //Negative luck = FANCY SANTAS LOL
								...
								if ($enemyrow['massiveresist'] != 100 && $damage > (floor($buddyrow[$maxhealthstr] / 100) * $enemyrow['massiveresist'])) { //Enemy resists massive damage applied.
									...
									if ($zilly >= 95) {
										...
										if ($damage > floor(($buddyrow[$maxhealthstr] / 100) * $enemyrow['massiveresist'] * 2))
											$damage = floor(($buddyrow[$maxhealthstr] / 100) * $enemyrow['massiveresist'] * 2);
									} else {
										$damage = floor(($buddyrow[$maxhealthstr] / 100) * $enemyrow['massiveresist']);
									}
								}
				...
				if ($buddyrow[$enemystr] != "") { //Enemy still exists after this round of combat. (sanity check: avoid weirdness if fragment does not exist)
					...
					if ($buddyrow['motifcounter'] == 0 || $buddyrow['Aspect'] != "Void") { //Void III stops positive effects the enemy generates, at least on the part near the Void player.
						...
						if (
							$enemyrow['boostdrain'] > 0 && ($buddyrow['powerboost'] > 0 || $buddyrow['offenseboost'] > 0 || $buddyrow['defenseboost'] > 0 || $buddyrow['temppowerboost'] > 0
								|| $buddyrow['tempoffenseboost'] > 0 || $buddyrow['tempdefenseboost'] > 0)
			...
			if ($buddyrow['Aspect_Vial'] >= $buddyrow['Gel_Viscosity'])
				$buddyrow['Aspect_Vial'] = $buddyrow['Gel_Viscosity'];
...
//kicks in at the end of the round)
		...
		if ($enemyrow['powerrecover'] > 0 && $powertotal < $sessionrow['sessionbossmaxpower']) { //Enemy recovers from some power loss every turn and has lost some.
			...
			if ($newpower > $sessionrow['sessionbossmaxpower'])
				$newpower = $sessionrow['sessionbossmaxpower'];
		...
		if ($enemyrow['healthrecover'] > 0 && $healthtotal < $sessionrow['sessionbossmaxhealth'] && $healthtotal > ($sessionrow['sessionbossmaxhealth'] / 20)) { //Enemy recovers from some health loss every turn and has lost some.
			...
			if ($newhealth > $sessionrow['sessionbossmaxhealth'])
				$newhealth = $sessionrow['sessionbossmaxhealth'];
			//$mysqli->query("UPDATE `Players` SET `" . $powerstr . "` = " . $newpower . " WHERE `Players`.`username` = '" . $username . "' LIMIT 1 ;");
		...
		while ($buddyrow = $sessionmates->fetch_array()) { //This loop normalizes and sets the focus for the next round
			if (empty($downarray[$buddyrow['username']]) && $buddyrow['sessionbossengaged'] == 1) {
				...
				if ($buddyrow['sessionbossleader'] == 1)
					$newfocus = $focus; //Display tweak.
