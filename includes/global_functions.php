...
function climbEcheladder(array $userrow, int $rungups)
{
	...
	for ($rungcounter = $rungs - 1; $rungcounter >= 0; $rungcounter--)
	{
		...
		if ($currentRung == 1) $hpup += 5; //First rung: +5
		elseif ($currentRung <= 5) $hpup += 10; //Rungs 2, 3, 4, and 5: +10
		else $hpup += 15; //Most rungs: +15
	}
	...
	if ($rungs > 0)
	{
		...
		{
			if ($rungs > 1) echo "</br>You scramble madly up your Echeladder, coming to rest on rung: $echerow[$echestr]!";
			else echo "</br>You ascend to rung: $echerow[$echestr]!";
		}
		
		foreach (fetchAll("SELECT * FROM Abilities WHERE Aspect IN (:aspect, 'All') AND Class IN (class,'All') AND Rungreq BETWEEN :rungReqLower AND :rungReqUpper AND Godtierreq = 0 ORDER BY Rungreq DESC;", [
			'aspect' => $userrow['Aspect'],
			'class' => $userrow['Class'],
			'rungReqLower' => $userrow['Echeladder'] + 1,
			'rungReqUpper' => $userrow['Echeladder'] + $rungs,
		]) as $levelerability)
...
function loadAbilities(array $userrow)
{
	...
	foreach (fetchAll("SELECT ID, Usagestr FROM Abilities WHERE Aspect IN (:aspect,'All') AND Class IN (:class,'All') AND Rungreq BETWEEN :rungReqLower AND :rungReqUpper AND Godtierreq BETWEEN :godTierReqLower AND :godTierReqUpper ORDER BY Rungreq DESC;", [
		'aspect' => $userrow['Aspect'],
		'class' => $userrow['Class'],
		'rungReqLower' => 0,
		'rungReqUpper' => $userrow['Echeladder'],
		'godTierReqLower' => 0,
		'godTierReqUpper' => $userrow['Godtier'],
	]) as $temp)
	{
