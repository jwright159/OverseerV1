...
switch ($userrow[$enemystr]) { //Catches specical case enemies with special interactions that aren't "gristed"
	...
	case "Kraken":
		...
		while ($gristlevel <= 3) {
			...
			$totalgrists = 0;
			while ($countrow = $gristtyperesult->fetch_array())
				$totalgrists++;
	...
	case "Screaming Hydra Head":
		...
		if ($roll < $splitchance) {
			...
			while ($hcount <= $max_enemies) {
				$headstr = "enemy" . strval($hcount) . "name";
				if (strpos($userrow[$headstr], "Hydra Head") !== false && $headstr != $enemystr)
					$moreheads = true;
				//the enemy that triggered this hasn't been blanked yet so don't count it
				elseif ($userrow[$headstr] == "Hydra")
					$thehydra = $headstr; //go ahead and set this as the body so we can blank it later
				else
					$nonhydrafoes = true;
				$hcount++;
			}
