	...
	switch ($userrow[$enemystr]) {
		case "Typheus":
			...
			if ($roll >= 90) {
				...
				if ($userrow['temppowerboost'] < 0 && $userrow['temppowerduration'] < 4)
					$userrow['temppowerduration'] = 4; //One round will be subtracted off later.
				$userrow['tempdefenseboost'] = $userrow['tempdefenseboost'] - $debuff;
				if ($userrow['tempdefenseboost'] < 0 && $userrow['tempdefenseduration'] < 4)
					$userrow['tempdefenseduration'] = 4; //One round will be subtracted off later.
			}
			break;
		case "Sophia":
		...
		case "Armok":
			...
			if ($playerdamage < 0 || $userrow['invulnerability'] > 0)
				$playerdamage = 0; //No healing the player with attacks!
			...
			if ($roll == 7) { //all hydra abilities trigger on a 1/7 chance.
				...
				if ($userrow['invulnerability'] == 0) {
					if ($damage > 250)
						...
						$damage = 250; //...and a 250 minimum if it didn't do much of anything
				}
			}
			break;
