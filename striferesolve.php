	...
	if ($userrow['enemy1name'] == "" && $userrow['enemy2name'] == "" && $userrow['enemy3name'] == "" && $userrow['enemy4name'] == "" && $userrow['enemy5name'] == "" && $userrow['aiding'] == "") { //User not actually strifing, get rid of them.
		...
		while ($row = $sessionmates->fetch_array()) {
			if ($row['aiding'] == $username) { //Aiding character.
				...
				if ($row['sprite_strength'] > 0 && $row['dreamingstatus'] == "Awake")
					$aidsprite += $row['sprite_strength'];
				$itemresult = $mysqli->query("SELECT * FROM Captchalogue WHERE `Captchalogue`.`name` = 'Perfectly Generic Object'");
				$blankrow = $itemresult->fetch_array();
				$aidblanks = 0;
				$aidvoidvalid = 0;
		...
		if (!empty($currentstatus)) { //here, we'll explode the currentstatus to see if we have any NPC allies
			...
			while (!empty($thisstatus[$st])) {
				...
				if ($statusarg[0] == "HASEFFECT") {
					...
					$thischarm = substr($statusarg[0], -1); //find out which enemy it is
		...
		while ($once || $repeat) {
				...
				if ($userrow[$enemystr] != "") { //Enemy spotted!
					...
					if ($i <= $targetedenemies) { //don't inflict damage/etc on an enemy that was spawned during this loop
						...
						if (!empty($abilities[23])) { //Fortune's Protection activates (ID 23). Regardless of other modifiers, grant a flat chance to score an instant critical. (Light guides your blow)
							...
							if ($guidance > (100 - floor($userrow['Echeladder'] / 50)))
						...
						$newenemyhealth = $userrow[$healthstr]; //derp.
					if ($newenemyhealth <= 0) { //Enemy defeated.
						...
						if (strpos($currentstatus, $timestopstr) !== false) { //This enemy is frozen in time.
							...
							if (strpos($currentstatus, $nocapstr) !== false) { //Player's massive damage cap is gone.
								...
								if ($playerdamage > ($userrow['Gel_Viscosity'] / 3))
									$playerdamage = floor($userrow['Gel_Viscosity'] / 3); //Massive damage safety net.
							}
			...
			if ($userrow[$healthvialstr] - $damage < $userrow['Gel_Viscosity']) {
				...
				if ($damage > 0)
					$loyaltydrain++; //allies lost some loyalty due to player being hit
			} else {
			...
			if ($userrow['motifcounter'] == 0 || $userrow['Aspect'] != "Time") { //Time III prevents boosts from ticking down.
				if ($userrow['temppowerduration'] > 0) {
					...
					if ($repeat)
						$powerlevel = $powerlevel - $userrow['temppowerboost']; //Remove boost in the case that we're repeating.
				...
				if ($userrow['tempoffenseduration'] > 0) {
					...
					if ($repeat)
						$offensepower = $offensepower - $userrow['tempoffenseboost']; //Remove boost in the case that we're repeating.
				...
				if ($userrow['tempdefenseduration'] > 0) {
					...
					if ($repeat)
						$defensepower = $defensepower - $userrow['tempdefenseboost']; //Remove boost in the case that we're repeating.
			...
			while ($row = $sessionmates->fetch_array()) {
				if ($row['aiding'] == $username) { //Aiding character. Reset their consumable use.
					if ($row['combatconsume'] != 0)
						$mysqli->query("UPDATE `Players` SET `combatconsume` = 0 WHERE `Players`.`username` = '" . $row['username'] . "' LIMIT 1 ;");
				...
				if ($userrow[$enemystr] != "") { //Enemy still exists after this round of combat.
					...
					if ($enemyrowexists) {
						...
						if (strpos($currentstatus, $unluckystr) !== false) { //This enemy is unlucky.
							...
							if ($misfortune != 1)
								$misfortune = floor($misfortune - ($luck / 10));
							if ($misfortune < 1)
			...
			if (!$alldead) {
				...
				while ($topshift <= $max_enemies) { //shift the status effect slot thingies down if enemies were removed so that the status effects still line up
					while (empty($userrow["enemy" . strval($topshift) . "name"]) && $topshift < $max_enemies)
						$topshift++;
			...
			if ($once)
				$once = false; //Don't run through more than once unless repeat is true.
			if ($nodamage == true)
				$nodamagecounter++;
			if ($nodamagecounter >= 5)
				$repeat = false; //No damage for five turns, stop repeating.
			if ($turncounter >= 20)
				$repeat = false; //20 turns, stop repeating.
		}
