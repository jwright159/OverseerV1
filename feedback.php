...
function getBonus($b)
{
	switch ($b) {
		...
		case 7:
			return "abstain";
		default:
			return "what the hell are you smoking";
	}
}
...
if (empty($_SESSION['username'])) {
	...
	if (!empty($_POST['itemsubmission'])) { //User is submitting an item.
		...
		if (empty($_POST['newitem']) && $editid != 0)
			$newitem = $mysqli->real_escape_string($fbrow['name']);
		else
			$newitem = $mysqli->real_escape_string($_POST['newitem']);
		if (empty($_POST['newdesc']) && $editid != 0)
			$newdesc = $mysqli->real_escape_string($fbrow['description']);
		else
			$newdesc = $mysqli->real_escape_string($_POST['newdesc']);
		if (empty($items) && $editid != 0)
			$items = $mysqli->real_escape_string($fbrow['recipe']);
		else
		...
		if (empty($_POST['power']) && $editid != 0)
			$newpower = $fbrow['power'];
		...
		if (empty($reccpower))
			$reccpower = 0;
		if (empty($_POST['other']) && $editid != 0)
			$newother = $mysqli->real_escape_string($fbrow['comments']);
		else
		...
		$size = "";
		if ($advanced) {
			if ($_POST['consumable'] == "yes")
				$consume = 1;
			if ($_POST['catalogue'] == "yes")
				$base = 1;
			if ($_POST['lootonly'] == "yes")
				$loot = 1;
			if ($_POST['refrance'] == "yes")
				$ref = 1;
			$i = 0;
			while ($i < 8) {
				$bname = getBonus($i);
			...
			if (!empty($_POST['abstratus']))
				$abstratus = $_POST['abstratus'];
			elseif ($editid != 0)
				$abstratus = $fbrow['abstratus'];
			else
				$abstratus = "notaweapon";
			$size = $_POST['size'];
		}
		if ($aok) {
			$editid = intval($_POST['editing']);
			if ($editid == 0) {
				...
				if ($fbrow['ID'] == $editid) {
					if (($fbrow['user'] == $username || $userrow['session_name'] == "Developers" || $userrow['session_name'] == "Itemods") && $fbrow['type'] == "item") {
						...
						echo "Submission error: Either that's not your submission, or you tried to edit a non-item.<br/>";
				} else
					echo "Submission error: the submission you tried to edit no longer exists.<br/>";
			}
		}
	}
