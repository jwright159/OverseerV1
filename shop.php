				...
				if (empty($_GET['land'])) {
					...
					if (!$aok)
						...
						if (empty($currentrow['shopstock']) || (time() - $currentrow['lastshoptick'] > 86400) || $forcerefresh) { //the shop is empty or a day has passed since the shop was last refreshed
							...
							if ($shopgate < 1)
								$shopgate = 1; //even on a land with no gates available, shops will stock as if there is at least one, in case the land is reached by a flying player for instance
						...
						if (!empty($_POST['buyitem'])) {
							if (empty($shopitem[$pid])) {
								$shoptext = "You're trying to buy a non-existant item!";
							} elseif ($userrow['Boondollars'] < $shopprice[$pid]) {
								$shoptext = "Sorry $username, I can't give credit! Come back when you're a little... mmm... RICHER!";
							} else {
								$shoptext = "Thank you for your purchase!";
							}
						}
						...
						while ($csi < $tsi) {
							...
							if ($canwield) {
								...
								if ($powerdiff < -9999) {
									$ramtext = "It looks like an utter piece of shit, though.";
								} elseif ($powerdiff < -5000) {
									$ramtext = "However, you outgrew the need for such flimsy weaponry long ago.";
								} elseif ($powerdiff < -1000) {
									$ramtext = "You doubt you could see any use out of it, though.";
								} elseif ($powerdiff < -100) {
									$ramtext = "It looks a bit weak for your needs.";
								} elseif ($powerdiff < 0) {
									$ramtext = "You think your current equipment might be better, but you never know...";
								} elseif ($powerdiff == 0) {
									$ramtext = "It looks exactly as strong as your current equipment.";
								} elseif ($powerdiff > 9999) {
									$ramtext = "Whoa, where has this been all your life?!";
								} elseif ($powerdiff > 9000) {
									$ramtext = "You'd be lucky to ever get your hands on one of these!";
								} elseif ($powerdiff > 5000) {
									$ramtext = "It looks ridiculously stronger than your current weapon!";
								} elseif ($powerdiff > 1000) {
									$ramtext = "You could definitely use something as strong as this!";
								} elseif ($powerdiff > 100) {
									$ramtext = "It looks pretty strong. It'd probably be a decent upgrade.";
								} elseif ($powerdiff > 0) {
							...
							?>
							<label class="conshop">
								<input type="radio" name="buyitem" id="buy<?php echo strval($csi); ?>" class="shpo"
									value="buy<?php echo strval($csi); ?>">
								<label class="gatcha">
									<span class="ripin">
										<?php echo $shopitem[$csi]; ?>
									</span>
									<div class="descripa">
										<?php echo $ramtext; ?>
									</div>
