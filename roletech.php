	...
	if ($userrow['Aspect_Vial'] >= $abilityrow['Aspect_Cost']) {
	  if ($userrow['combatconsume'] == 0 || $strifing == False) { //User has an action or isn't strifing
	    ...
	    case 6: //Esauna
	      if ($targetfound) {
		echo "$abilityrow[Usagestr]</br>";
		if ($targetrow['powerboost'] < 0) $mysqli->query("UPDATE `Players` SET `powerboost` = 0 WHERE `Players`.`username` = '$target' LIMIT 1 ;");
		if ($targetrow['offenseboost'] < 0) $mysqli->query("UPDATE `Players` SET `offenseboost` = 0 WHERE `Players`.`username` = '$target' LIMIT 1 ;");
		if ($targetrow['defenseboost'] < 0) $mysqli->query("UPDATE `Players` SET `defenseboost` = 0 WHERE `Players`.`username` = '$target' LIMIT 1 ;");
		if ($targetrow['temppowerboost'] < 0) $mysqli->query("UPDATE `Players` SET `temppowerboost` = 0 WHERE `Players`.`username` = '$target' LIMIT 1 ;");
		if ($targetrow['tempoffenseboost'] < 0) $mysqli->query("UPDATE `Players` SET `tempoffenseboost` = 0 WHERE `Players`.`username` = '$target' LIMIT 1 ;");
		if ($targetrow['tempdefenseboost'] < 0) $mysqli->query("UPDATE `Players` SET `tempdefenseboost` = 0 WHERE `Players`.`username` = '$target' LIMIT 1 ;");
		...
		$mysqli->query("UPDATE `Players` SET `strifestatus` = '$currentstatus' WHERE `Players`.`username` = '" . $target . "' LIMIT 1 ;");
	      }
	      break;
	    ...
	    if ($abilityrow['targets'] != 1 || $targetfound) { //Counts as an action if a target was found or no targeting happens.
	      if ($strifing) $mysqli->query("UPDATE `Players` SET `combatconsume` = 1 WHERE `Players`.`username` = '" . $username . "' LIMIT 1 ;"); //Only do this if strifing.
