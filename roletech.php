							...
							case 6: //Esauna
								if ($targetfound) {
									...
									if ($targetrow['offenseboost'] < 0)
										$mysqli->query("UPDATE `Players` SET `offenseboost` = 0 WHERE `Players`.`username` = '$target' LIMIT 1 ;");
									if ($targetrow['defenseboost'] < 0)
										$mysqli->query("UPDATE `Players` SET `defenseboost` = 0 WHERE `Players`.`username` = '$target' LIMIT 1 ;");
									if ($targetrow['temppowerboost'] < 0)
										$mysqli->query("UPDATE `Players` SET `temppowerboost` = 0 WHERE `Players`.`username` = '$target' LIMIT 1 ;");
									if ($targetrow['tempoffenseboost'] < 0)
										$mysqli->query("UPDATE `Players` SET `tempoffenseboost` = 0 WHERE `Players`.`username` = '$target' LIMIT 1 ;");
									if ($targetrow['tempdefenseboost'] < 0)
