...
if (empty($_SESSION['username'])) {
	...
	if ($userrow['enemydata'] != "" || $userrow['aiding'] != "") {
		if ($userrow['hascomputer'] < 3) {
			if ($compugood == true)
				echo "You don't have a hands-free computer equipped, so you can't send or view messages during strife.<br/>";
			$compugood = false;
		}
	}
	if ($userrow['indungeon'] != 0 && $userrow['hascomputer'] < 2) {
		if ($compugood == true)
			echo "You don't have a portable computer in your inventory, so you can't send or view messages while away from home.<br/>";
		$compugood = false;
	}
	if ($userrow['hascomputer'] == 0) {
		if ($compugood == true)
			echo "You need a computer in storage or your inventory to send and view messages.<br/>";
		$compugood = false;
	}
	...
	if ($compugood) {
		...
		if (!empty($_POST['msgfix'])) {
			...
			while ($counter <= 50) {
				...
				if (empty($boom[3]) && !empty($msgstring))
					$unreads++;
				$counter++;
			}
		...
		while ($k <= $max_inbox) {
			if ($msgrow['msg' . strval($k)] != '' && empty($deleted[$k])) {
				...
				$fixsubject = str_replace("THIS IS A LINE", "|", $boom[1]);
				if (empty($boom[3]) && empty($marked[$k]))
					echo '<b>';
