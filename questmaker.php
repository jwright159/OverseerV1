...
if (empty($username)) {
	...
	if (!empty($_POST['qprompt'])) {
		$dialogue = $mysqli->real_escape_string($_POST['qprompt']);
		$keywords = $mysqli->real_escape_string($_POST['qkeywords']);
		$abstratus = $mysqli->real_escape_string($_POST['qabstratus']);
		$grist = $mysqli->real_escape_string($_POST['qgrists']);
		if (!empty($_POST['qpower']))
			$power = $_POST['qpower'];
		else
			$power = 0;
		$base = $mysqli->real_escape_string($_POST['qbase']);
		$consume = $mysqli->real_escape_string($_POST['qconsume']);
		$size = $mysqli->real_escape_string($_POST['qsize']);
		$reward = $mysqli->real_escape_string($_POST['qreward']);
		...
		$mysqli->query("INSERT INTO `Consort_Dialogue` (`dialogue`, `context`, `gate`, `req_keyword`, `req_abstratus`, `req_grist`, `req_base`, `req_consume`, `req_power`, `req_size`, `specialreward`) VALUES ('$dialogue', 'quest', '$gate', '$keywords', '$abstratus', '$grist', '$base', '$consume', $power, '$size', '$reward')");
	...
	echo '(rewards must be put in mysqli query search format, example: `Captchalogue`.`power` > 100 will search for any item greater than 100 power. Leave blank for default; see Blah if you need help figuring out what to put for a particular query)<br/>';
