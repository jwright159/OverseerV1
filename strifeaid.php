		...
		if ($aidrow['session_name'] != $userrow['session_name']) {
			...
			if (!empty($aidrow['enemy1name'])) {
				...
				while ($enemyrow = $enemyresult->fetch_array()) {
					if ($enemyrow['appearson'] == "Battlefield")
						$onbattlefield = true;
				}
			} //removed all the others because the first enemy will always be in slot 1
			if ($onbattlefield) { //and if so...
				if ($userrow['battlefield_access'] == 1)
					$aok = true; //only admit the user if they have battlefield access as well
				else
					$aok = false;
			}
			if ($aok) {
