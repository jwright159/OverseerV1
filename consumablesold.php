...
if (empty($_SESSION['username'])) {
	...
	if (!empty($_POST['consume'])) { //Consuming time!
			...
			switch ($userrow[$_POST['consume']]) { //Determine the consumable and act accordingly.
				case "Medkit":
					if ($userrow['aiding'] == "") { //User not currently aiding.
						...
						$heal = 100;
						if ($userrow['Health_Vial'] + $heal > $userrow['Gel_Viscosity'])
							$heal = ($userrow['Gel_Viscosity'] - $userrow['Health_Vial']); //No overheals.
						...
						$heal = 175;
						if ($allyrow['Health_Vial'] + $heal > $allyrow['Gel_Viscosity'])
							$heal = ($allyrow['Gel_Viscosity'] - $allyrow['Health_Vial']); //No overheals.
			   ...
			   //This will put you to sleep, giving your dream self a huge power boost.
				...
				case "Shamrock Block":
					...
					$luckboost = 5;
					if ($userrow['Brief_Luck'] + $luckboost > 100)
						$luckboost = (100 - $userrow['Brief_Luck']);
		...
		if (!$fail && !$donotconsume) {
			...
			if ($userrow['equipped'] == $_POST['consume'])
				$mysqli->query("UPDATE `Players` SET `equipped` = '' WHERE `Players`.`username` = '" . $username . "' LIMIT 1 ;"); //If consumable equipped or offhand
			if ($userrow['offhand'] == $_POST['consume'])
				$mysqli->query("UPDATE `Players` SET `offhand` = '' WHERE `Players`.`username` = '" . $username . "' LIMIT 1 ;"); //...then unequip that slot.
		}
	}
