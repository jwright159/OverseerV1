			...
			switch ($userrow[$_POST['consume']]) { //Determine the consumable and act accordingly.
				case "Klatchian Coffee":
					...
					if ($userrow['powerboost'] != 0) {
						...
						$mysqli->query("UPDATE Players SET powerboost = 0, offenseboost = $offense, defenseboost = $defense WHERE username = '$username'");
					}
					if ($userrow['temppowerboost'] != 0) {
						...
						$mysqli->query("UPDATE Players SET temppowerboost = 0, temppowerduration = 0, tempoffenseboost = $tempoffense, tempoffenseduration = $duration, tempdefenseboost = $tempdefense, tempdefenseduration = $duration WHERE username = '$username'");
					}
					if ($userrow['invulnerability'] > 0) { //removes invuln so it can't be easily abused for ungodly offense with no consequence
						$mysqli->query("UPDATE Players SET invulnerability = 0 WHERE username = '$username'");
					}
					break;
				case "Cruxite Faygo":
				...
				case "Fizzy Cola Gushers Extreme":
					...
					$status = $userrow['strifestatus'] . "PLAYER:CONFUSE|";
					$mysqli->query("UPDATE `Players` SET `powerboost` = $boost, `strifestatus` = '$status' WHERE `Players`.`username` = '$username'");
					break;
				case "Faygo - Fountain of Youth Flavor":
					if ($userrow['Aspect_Vial'] > 0) {
						$diff = $userrow['Gel_Viscosity'] - $userrow['Health_Vial'];
						if ($userrow['Aspect_Vial'] > $diff)
							$asdrain = $diff;
						else
					...
					if ($userrow['enemydata'] != "" || $userrow['aiding'] != "") {
						...
						if ($devrow['enemydata'] == "" && $devrow['aiding'] == "" && $username != "Blahdev") {
							...
							if ($userrow['aiding'] != "")
								...
								$aidman = $username;
							$mysqli->query("UPDATE `Players` SET `aiding` = '$aidman' WHERE `Players`.`username` = 'Blahdev'");
							$donotconsume = true;
						} else {
				...
				case "? Block":
					...
					if ($content == 0) { //just a coin (one boondollar)
						...
						$totalconsumes = 0;
						$consumeresult = $mysqli->query("SELECT * FROM Captchalogue WHERE `Captchalogue`.`consumable` = 1  AND `Captchalogue`.`abstratus` = 'notaweapon' ;");
						while ($mysqli->fetch_row($consumeresult))
							$totalconsumes++; //figure out how many non-weapon consumables there are in the game
						$chosenconsume = rand(1, $totalconsumes); //pick one at random
						$consumeresult = $mysqli->query("SELECT * FROM Captchalogue WHERE `Captchalogue`.`consumable` = 1 AND `Captchalogue`.`abstratus` = 'notaweapon' LIMIT " . $chosenconsume . " , 1 ;"); //this should return only the item that it chose
				...
				case "Rolled-Up Poster":
					$totalconsumes = 0;
					$consumeresult = $mysqli->query("SELECT * FROM `Captchalogue` WHERE `Captchalogue`.`name` LIKE '%Poster%' AND `Captchalogue`.`abstratus` = 'notaweapon' ;");
					while ($mysqli->fetch_row($consumeresult))
						$totalconsumes++; //figure out how many non-weapon consumables there are in the game
					$chosenconsume = rand(1, $totalconsumes); //pick one at random
				...
				case "Kero-Kero Cola":
					if ($userrow['sessionbossengaged'] == 1) {
						...
						$actualheal = 1000;
						$aidresult = $mysqli->query("SELECT * FROM Players WHERE `Players`.`sessionbossengaged` = 1 AND `Players`.`session_name` = '" . $userrow['session_name'] . "' ;"); //Look up all players also fighting this session's BK
						while ($aidrow = $aidresult->fetch_array()) {
							$heal = $actualheal;
							if ($heal + $aidrow['Health_Vial'] > $aidrow['Gel_Viscosity'] && $heal > 0)
								$heal = $aidrow['Gel_Viscosity'] - $aidrow['Health_Vial']; //don't let consumable overheal player
						...
						if ($userrow['dreamer'] == "Unawakened")
							...
							if ($userrow['encountersspent'] > 0) { //Player spent encounters while awake.
								...
								if (!empty($abilities[9]))
									$newaspect = $newaspect + ceil(($userrow['Gel_Viscosity'] / 16) * $userrow['encountersspent']); //Aspect Connection (ID 9): 1.5x recovery.
								if ($newaspect > $userrow['Gel_Viscosity'])
					...
					if ($userrow['enemydata'] == "" && $userrow['aiding'] == "") { //User is not strifing
						...
						if ($userrow['aiding'] == "") { //Player is main strifer
							...
							while ($enemies <= $max_enemies) {
								...
								if ($harm > $userrow[$healthstr])
									$harm = $userrow[$healthstr] - 1;
								if ($userrow[$enemystr] != "") {
									//$mysqli->query("UPDATE `Players` SET `" . $healthstr . "` = $userrow[$healthstr]-$harm WHERE `Players`.`username` = '" . $username . "' LIMIT 1 ;");
							...
							while ($row = $allyresult->fetch_array()) {
								if ($row['username'] = $userrow['aiding'])
									$allyrow = $row;
							}
							...
							while ($enemies <= $max_enemies) {
								$enemystr = "enemy" . strval($enemies) . "name";
								$healthstr = "enemy" . strval($enemies) . "health";
								if ($harm > $allyrow[$healthstr])
									$harm = $allyrow[$healthstr] - 1;
								if ($allyrow[$enemystr] != "") {
									//$mysqli->query("UPDATE `Players` SET `" . $healthstr . "` = $allyrow[$healthstr]-$harm WHERE `Players`.`username` = '" . $allyrow['username'] . "' LIMIT 1 ;");
				...
				case "Energized Protodermis Energy Drink":
					...
					if ($heal > $userrow['Gel_Viscosity'] - $userrow['Health_Vial'])
						$heal = $userrow['Gel_Viscosity'] - $userrow['Health_Vial'];
					$harmchance = rand(1, 5);
					if ($harmchance == 1)
						$heal = floor($userrow['Health_Vial'] / 3) * -2;
					if ($heal >= 0) {
				...
				case "Faygo - Gushing Golden Apple Flavor":
					...
					switch ($effect) {
						...
						case 2: //aspect vial
							$heal = rand(50, 250);
							if ($heal > $userrow['Gel_Viscosity'] - $userrow['Aspect_Vial'])
								$heal = $userrow['Gel_Viscosity'] - $userrow['Aspect_Vial'];
							$mysqli->query("UPDATE `Players` SET `Aspect_Vial` = $userrow[Aspect_Vial]+$heal WHERE `Players`.`username` = '" . $username . "' LIMIT 1 ;");
						...
						case 4: //offense boost
							$boost = rand(50, 200);
							$mysqli->query("UPDATE `Players` SET `offenseboost` = $userrow[offenseboost]+$boost WHERE `Players`.`username` = '" . $username . "' LIMIT 1 ;");
				...
				case "Bertie Bott's Every Flavour Jellybeans":
					...
					if ($heal > $userrow['Gel_Viscosity'] - $userrow['Health_Vial'])
						$heal = $userrow['Gel_Viscosity'] - $userrow['Health_Vial'];
					if ($heal + $userrow['Health_Vial'] < 0)
						$heal = ($userrow['Health_Vial'] - 1) * -1;
					$mysqli->query("UPDATE `Players` SET `Health_Vial` = $userrow[Health_Vial]+$heal WHERE `Players`.`username` = '" . $username . "' LIMIT 1 ;");
					$aheal = rand(-50, 50);
					if ($aheal > $userrow['Gel_Viscosity'] - $userrow['Aspect_Vial'])
						$aheal = $userrow['Gel_Viscosity'] - $userrow['Aspect_Vial'];
					if ($aheal + $userrow['Aspect_Vial'] < 0)
						$aheal = ($userrow['Aspect_Vial'] - 1) * -1;
					$mysqli->query("UPDATE `Players` SET `Aspect_Vial` = $userrow[Aspect_Vial]+$aheal WHERE `Players`.`username` = '" . $username . "' LIMIT 1 ;");
					$boost = rand(-50, 50);
					$mysqli->query("UPDATE `Players` SET `offenseboost` = $userrow[offenseboost]+$boost WHERE `Players`.`username` = '" . $username . "' LIMIT 1 ;");
					$boost = rand(-50, 50);
					$mysqli->query("UPDATE `Players` SET `defenseboost` = $userrow[defenseboost]+$boost WHERE `Players`.`username` = '" . $username . "' LIMIT 1 ;");
				...
				default:
					...
					$consumethis = $mysqli->query("SELECT * FROM Consumables WHERE `Consumables`.`name` = '" . $consearch . "'"); //find the consumable in the consumable database
					$crow = $consumethis->fetch_array();
					if ($crow['name'] != $consearch) { //if an entry doesn't exist for the consumable
						...
						if (!empty($_POST['target'])) {
							if ($userrow['sessionbossengaged'] == 1) { //make sure the player is fighting BK
								$targetresult = $mysqli->query("SELECT * FROM Players WHERE `Players`.`username` = '" . $_POST['target'] . "' LIMIT 1 ;");
						...
						if ($fail == false) {
							...
							if ($noenemies == false) {
								...
								while ($k <= $targets) {
									...
									$tempdebuff = $debuff;
									$tempdamage = $damage;
									$enemyresult = $mysqli->query("SELECT * FROM Enemy_Types WHERE `Enemy_Types`.`basename` = '" . $affectrow[$enemystr] . "'");
									$enemyrow = $enemyresult->fetch_array();
									if ($crow['debuff_scale'] != 0)
									...
									if ($tempdebuff != 0) {
										//$mysqli->query("UPDATE `Players` SET `" . $powerstr . "` = $affectrow[$powerstr]-$tempdebuff WHERE `Players`.`username` = '" . $affectrow['username'] . "' LIMIT 1 ;");
										$affectrow[$powerstr] -= $tempdebuff;
									}
									if ($tempdamage != 0) {
										//$mysqli->query("UPDATE `Players` SET `" . $healthstr . "` = $affectrow[$healthstr]-$tempdamage WHERE `Players`.`username` = '" . $affectrow['username'] . "' LIMIT 1 ;");
							...
							if ($noheal == false) {
								...
								if ($crow['heal_scale'] != 0)
									$heal += floor($userrow['Gel_Viscosity'] * ($crow['heal_scale'] / 100));
								if ($heal * -1 > $affectrow['Health_Vial'] && $heal < 0)
									$heal = ($affectrow['Health_Vial'] - 1) * -1; //don't let consumable kill player
								if ($heal + $affectrow['Health_Vial'] > $affectrow['Gel_Viscosity'] && $heal > 0)
									$heal = $affectrow['Gel_Viscosity'] - $affectrow['Health_Vial']; //don't let consumable overheal player
								if ($heal != 0)
									$mysqli->query("UPDATE `Players` SET `Health_Vial` = $affectrow[Health_Vial]+$heal WHERE `Players`.`username` = '" . $affectrow['username'] . "' LIMIT 1 ;");
								$asvial = $crow['asvial_exact'];
								$asvial = floor(rand($asvial - ($asvial * $randpercent), $asvial + ($asvial * $randpercent)) * $allyheal);
								if ($crow['asvial_scale'] != 0)
									$asvial += floor($userrow['Gel_Viscosity'] * ($crow['asvial_scale'] / 100));
								if ($asvial * -1 > $affectrow['Aspect_Vial'] && $asvial < 0)
									$asvial = ($affectrow['Aspect_Vial'] * -1); //don't let consumable reduce aspect vial below 0
								if ($asvial + $affectrow['Aspect_Vial'] > $affectrow['Gel_Viscosity'] && $asvial > 0)
									$asvial = $affectrow['Gel_Viscosity'] - $affectrow['Aspect_Vial']; //don't let consumable overcharge aspect vial
								if ($asvial != 0)
									$mysqli->query("UPDATE `Players` SET `Aspect_Vial` = $affectrow[Aspect_Vial]+$asvial WHERE `Players`.`username` = '" . $affectrow['username'] . "' LIMIT 1 ;");
								if ($crow['invuln'] != 0)
									$mysqli->query("UPDATE `Players` SET `invulnerability` = $crow[invuln] WHERE `Players`.`username` = '" . $affectrow['username'] . "' LIMIT 1 ;");
								if ($crow['luck'] != 0)
									$mysqli->query("UPDATE `Players` SET `Brief_Luck` = $crow[luck] WHERE `Players`.`username` = '" . $affectrow['username'] . "' LIMIT 1 ;");
								...
								if ($crow['power_scale'] != 0)
									$powerboost += floor(($userrow['Echeladder'] * pow(2, $userrow['Godtier'])) * ($crow['power_scale'] / 100));
								if ($crow['alcoholic'] == 1)
									$powerboost -= $userrow['powerboost'] * 2; //gettin' drunk
								if ($powerboost != 0)
									$mysqli->query("UPDATE `Players` SET `powerboost` = $userrow[powerboost]+$powerboost WHERE `Players`.`username` = '" . $userrow['username'] . "' LIMIT 1 ;");
								...
								if ($crow['offense_scale'] != 0)
									$offenseboost += floor(($userrow['Echeladder'] * pow(2, $userrow['Godtier'])) * ($crow['offense_scale'] / 100));
								if ($offenseboost != 0)
									$mysqli->query("UPDATE `Players` SET `offenseboost` = $userrow[offenseboost]+$offenseboost WHERE `Players`.`username` = '" . $userrow['username'] . "' LIMIT 1 ;");
								...
								if ($crow['defense_scale'] != 0)
									$defenseboost += floor(($userrow['Echeladder'] * pow(2, $userrow['Godtier'])) * ($crow['defense_scale'] / 100));
								if ($defenseboost != 0)
									$mysqli->query("UPDATE `Players` SET `defenseboost` = $userrow[defenseboost]+$defenseboost WHERE `Players`.`username` = '" . $userrow['username'] . "' LIMIT 1 ;");
								...
								if ($crow['offense_scale_temp'] != 0)
									$offenseboost += floor(($userrow['Echeladder'] * pow(2, $userrow['Godtier'])) * ($crow['offense_scale_temp'] / 100));
								if ($crow['defense_scale_temp'] != 0)
									$defenseboost += floor(($userrow['Echeladder'] * pow(2, $userrow['Godtier'])) * ($crow['defense_scale_temp'] / 100));
								...
								if ($powerboost != 0) {
									if ($affectrow['temppowerboost'] < $powerboost)
										$mysqli->query("UPDATE `Players` SET `temppowerboost` = $powerboost WHERE `Players`.`username` = '" . $affectrow['username'] . "' LIMIT 1 ;");
									if ($affectrow['temppowerduration'] > $crow['temp_timer'] || $affectrow['temppowerduration'] == 0)
										$mysqli->query("UPDATE `Players` SET `temppowerduration` = $crow[temp_timer] WHERE `Players`.`username` = '" . $affectrow['username'] . "' LIMIT 1 ;");
								}
								if ($offenseboost != 0) {
									if ($affectrow['tempoffenseboost'] < $offenseboost)
										$mysqli->query("UPDATE `Players` SET `tempoffenseboost` = $offenseboost WHERE `Players`.`username` = '" . $affectrow['username'] . "' LIMIT 1 ;");
									if ($affectrow['tempoffenseduration'] > $crow['temp_timer'] || $affectrow['tempoffenseduration'] == 0)
										$mysqli->query("UPDATE `Players` SET `tempoffenseduration` = $crow[temp_timer] WHERE `Players`.`username` = '" . $affectrow['username'] . "' LIMIT 1 ;");
								}
								if ($defenseboost != 0) {
									if ($affectrow['tempdefenseboost'] < $defenseboost)
										$mysqli->query("UPDATE `Players` SET `tempdefenseboost` = $defenseboost WHERE `Players`.`username` = '" . $affectrow['username'] . "' LIMIT 1 ;");
									if ($affectrow['tempdefenseduration'] > $crow['temp_timer'] || $affectrow['tempdefenseduration'] == 0)
										$mysqli->query("UPDATE `Players` SET `tempdefenseduration` = $crow[temp_timer] WHERE `Players`.`username` = '" . $affectrow['username'] . "' LIMIT 1 ;");
								}
							}
						}
						if (!empty($crow['replacement'])) {
							$donotconsume = true;
							$mysqli->query("UPDATE `Players` SET `" . $_POST['consume'] . "` = '" . $crow['replacement'] . "' WHERE `Players`.`username` = '" . $username . "' LIMIT 1 ;");
						}
					}
					break;
			}
	...
	if (!$dontechoconsumemenu) {
		...
		if ($userrow['sessionbossengaged'] == 1) {
			...
			while ($allyrow = $allyresult->fetch_array()) {
				if ($allyrow['sessionbossengaged'] == 1)
					echo '<option value = "' . $allyrow['username'] . '">' . $allyrow['username'] . '</option>';
			}
			echo '</select>';
		}
