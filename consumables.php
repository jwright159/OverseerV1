  ...
  if (!empty($_POST['consume'])) { //Consuming time!
	...
	if ($content == 0) { //just a coin (one boondollar)
	  ...
	  $mysqli->query("UPDATE `Players` SET `Boondollars` = " . strval($userrow['Boondollars']+1) . " WHERE `Players`.`username` = '" . $username . "' LIMIT 1 ;");
	  	...
	  	while ($mysqli->fetch_row($consumeresult)) $totalconsumes++; //figure out how many non-weapon consumables there are in the game
	...
	if ($crow['name'] != $consearch) { //if an entry doesn't exist for the consumable
		...
		if ($noheal == False) {
		    ...
		    if ($heal != 0) $mysqli->query("UPDATE `Players` SET `Health_Vial` = $affectrow[Health_Vial]+$heal WHERE `Players`.`username` = '" . $affectrow['username'] . "' LIMIT 1 ;");
		    ...
		    if ($asvial != 0) $mysqli->query("UPDATE `Players` SET `Aspect_Vial` = $affectrow[Aspect_Vial]+$asvial WHERE `Players`.`username` = '" . $affectrow['username'] . "' LIMIT 1 ;");
		    if ($crow['invuln'] != 0) $mysqli->query("UPDATE `Players` SET `invulnerability` = $crow[invuln] WHERE `Players`.`username` = '" . $affectrow['username'] . "' LIMIT 1 ;");
		    if ($crow['luck'] != 0) $mysqli->query("UPDATE `Players` SET `Brief_Luck` = $crow[luck] WHERE `Players`.`username` = '" . $affectrow['username'] . "' LIMIT 1 ;");
		    ...
		    if ($powerboost != 0) $mysqli->query("UPDATE `Players` SET `powerboost` = $userrow[powerboost]+$powerboost WHERE `Players`.`username` = '" . $userrow['username'] . "' LIMIT 1 ;");
		    ...
		    if ($offenseboost != 0) $mysqli->query("UPDATE `Players` SET `offenseboost` = $userrow[offenseboost]+$offenseboost WHERE `Players`.`username` = '" . $userrow['username'] . "' LIMIT 1 ;");
		    ...
		    if ($defenseboost != 0) $mysqli->query("UPDATE `Players` SET `defenseboost` = $userrow[defenseboost]+$defenseboost WHERE `Players`.`username` = '" . $userrow['username'] . "' LIMIT 1 ;");
		    ...
		    if ($powerboost != 0) {
			if ($affectrow['temppowerboost'] < $powerboost) $mysqli->query("UPDATE `Players` SET `temppowerboost` = $powerboost WHERE `Players`.`username` = '" . $affectrow['username'] . "' LIMIT 1 ;");
			if ($affectrow['temppowerduration'] > $crow['temp_timer'] || $affectrow['temppowerduration'] == 0) $mysqli->query("UPDATE `Players` SET `temppowerduration` = $crow[temp_timer] WHERE `Players`.`username` = '" . $affectrow['username'] . "' LIMIT 1 ;");
			}
		    if ($offenseboost != 0) {
			if ($affectrow['tempoffenseboost'] < $offenseboost) $mysqli->query("UPDATE `Players` SET `tempoffenseboost` = $offenseboost WHERE `Players`.`username` = '" . $affectrow['username'] . "' LIMIT 1 ;");
			if ($affectrow['tempoffenseduration'] > $crow['temp_timer'] || $affectrow['tempoffenseduration'] == 0) $mysqli->query("UPDATE `Players` SET `tempoffenseduration` = $crow[temp_timer] WHERE `Players`.`username` = '" . $affectrow['username'] . "' LIMIT 1 ;");
			}
		    if ($defenseboost != 0) {
			if ($affectrow['tempdefenseboost'] < $defenseboost) $mysqli->query("UPDATE `Players` SET `tempdefenseboost` = $defenseboost WHERE `Players`.`username` = '" . $affectrow['username'] . "' LIMIT 1 ;");
			if ($affectrow['tempdefenseduration'] > $crow['temp_timer'] || $affectrow['tempdefenseduration'] == 0) $mysqli->query("UPDATE `Players` SET `tempdefenseduration` = $crow[temp_timer] WHERE `Players`.`username` = '" . $affectrow['username'] . "' LIMIT 1 ;");
			}
		    }
		}
