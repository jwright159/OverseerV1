...
if (empty($_SESSION['username'])) {
	...
	if (empty($equippedhead))
		$equippedhead = "";
	if (empty($equippedface))
		$equippedface = "";
	if (empty($equippedbody))
		$equippedbody = "";
	if (empty($equippedacc))
		$equippedacc = "";
	if (empty($headname)) {
		if ($userrow['headgear'] != "") {
			...
			if ($itemname == $userrow[$invslot]) {
				...
				if (strrpos($row['abstratus'], "headgear") || strrpos($row['abstratus'], "facegear") || strrpos($row['abstratus'], "bodygear") || strrpos($row['abstratus'], "accessory")) {
					...
					if ($row['hybrid'] == 1) {
						...
						if ($hfadef != 0 && $boddef != 0)
							$mypower = strval($hfadef) . " (" . strval($boddef) . " on body)";
						elseif ($hfadef != 0)
							$mypower = strval($hfadef);
						elseif ($boddef != 0)
							$mypower = strval($boddef);
						else
							$mypower = "0";
						if ($bhfadef != 0 && $bboddef != 0)
							$mybpower = strval($bhfadef) . " (" . strval($bboddef) . " on body)";
						elseif ($bhfadef != 0)
							$mybpower = strval($bhfadef);
						elseif ($bboddef != 0)
							$mybpower = strval($bboddef);
						else
							$mybpower = "0";
					}
